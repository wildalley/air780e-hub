...
- 设备详情新增只读网络诊断，类型化调用 `AT+CCED`、`AT+EEMGINFO`、`AT*BANDIND?` 与 `AT^SYSINFO` 并保留不同固件的原始返回；单条指令不支持时仍展示其余结果。字段含义手册未给出，因此按原始行展示而不解析成结构化字段。设备页仅显示 Agent 实际上报的小节，落后一个版本的 Agent 不会显示为空。
