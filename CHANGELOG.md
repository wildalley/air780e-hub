...
- Server 数据库 schema 从 v2 顺序迁移到 v6，启动前自动写入 `hub.db.v<旧版本>.bak` 快照；迁移会重新标记库内已有的数据短信，并为 SIM 增加计费、余额、低余额阈值、套餐到期和保号截止字段。该版本新增 Agent/Server 协议 v1，部署时必须从同一提交升级并重启 Server 与 Agent；只同步 Agent 源码而不重新执行 `pip install` 仍会运行旧副本。
