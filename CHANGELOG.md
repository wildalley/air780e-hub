...
- 设备详情新增官方 AT 文档覆盖的运营商扫描与选择：`AT+COPS=?` 扫描可见网络，按 MCC/MNC 手动选择，或用 `AT+COPS=0` 恢复自动选网；长扫描使用独立超时，手动模式下自动注册自愈不会撤销管理员选择。
- 设备详情新增只读网络诊断，类型化调用 `AT+CCED=0,1`（服务小区）、`AT+CCED=0,2`（邻区）、`AT+EEMGINFO`、`AT*BANDIND?` 与 `AT^SYSINFO` 并保留不同固件的原始返回；单条指令不支持时仍展示其余结果。字段含义手册未给出，因此按原始行展示而不解析成结构化字段。设备页仅显示 Agent 实际上报的小节，落后一个版本的 Agent 不会显示为空。
- 文档记录 Air780E `V1011` 实测确认没有可用的锁频、锁小区接口：`*BANDIND` 唯一可写形式是 `(0,1)` 开关，`^SYSCONFIG` 制式字段只允许 `(2)`，`^CACAP` 与 `AT+CPOL` 不支持。Web 因此不会下发从其它芯片移植来的私有命令。
- 每张 SIM 可设置低余额阈值；余额大于 0 且不高于阈值时创建 warning 运维事件，余额为 0 或负数时升级为 critical，充值恢复、清空余额或清空阈值后自动关闭。金额使用十进制字符串存储和比较，不经过浮点数。
- SIM 卡支持结构化维护计费方式、套餐名称、PAYG/预付费余额与币种、套餐到期日和保号截止日；余额变更时间由 Server 记录，两个期限分别在 30 天内创建 warning 运维事件，7 天内或已过期升级为 critical，并独立关闭。
...
- 网络诊断此前发送裸 `AT+CCED`，V1011 一律返回 `+CME ERROR: 3`，看起来像固件不支持，实际只是缺少 `<mode>,<dump>` 参数。现在按 `=0,1` 和 `=0,2` 分别读服务小区与邻区，只用单次读取形式，不开启周期上报。`AT+EEMGINFO` 在 V1011 上确认不存在（`+`/`^`/`*` 及 `+EMGINFO` 五种拼法连测试形式都返回 `ERROR`），保留调用以便支持该指令的固件仍能上报。
