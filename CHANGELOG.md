...
- Server 新增持久化通知 outbox、命令作业资源和任务配置同步回执；长操作在刷新或重启后仍可查询，短信/呼叫在结果未确认时明确标记为未知，不会因 HTTP 超时透明重发硬件命令。数据库恢复进入维护模式，协调 Agent、通知、告警、任务同步和会话状态，并在切换失败时回退到恢复前快照。
- 新增公开 `/readyz` 就绪检查和管理员运行指标：按业务时区统计今日短信与趋势，诊断页提供 HTTP、数据库、ACK、通知 provider 和 WebSocket 关闭原因的受控进程指标；指标不缓存、不记录号码、凭据、正文或对端关闭文本。
- Agent/Server 事件链路增加事件流代际、设备唯一身份、提交后 ACK、有限并发命令队列和持久化命令结果；同名设备不会被错误路由，重复事件和迟到回执不会重复应用或覆盖已确认结果。

- Agent 每次状态采样读取 `AT+CBC` 上报模块供电电压，设备详情显示电压并在偏低时给出告警，历史曲线按最低值聚合以保留电压跌落。低于阈值创建 warning 运维事件，低于 3300 mV 升级为 critical，恢复后自动关闭。判定阈值 `low_voltage_mv` 按模块配置并随状态帧上报——多低算低取决于这块模块的供电方式，Server 只负责判定，不保存第二份默认值。取不到读数时不动已有告警，沉默不算恢复。
- 保号任务新增 `voice_call` 动作：拨号后等待若干秒响铃再立即挂断，不会真正接通。判定成功的依据是运营商是否受理了这次呼叫，因此 `BUSY` 与 `NO ANSWER` 算成功，只是拨不出去（无网络、模块未受理）才算失败并重试。呼叫期间轮询 `AT+CLCC` 记录进展，结果写入任务日志。号码只接受数字与 `+*#ABCD`，避免拨号串里夹回车拼接出第二条 AT 指令。
- Agent 记录来电：开启 `AT+CLIP=1` 后把 `RING` 与来电号码写入日志，同一次来电只记一条，从不接听。
...
- 修复数据库读写调度、IANA 时区分桶以及运行指标接入中的回归；长读、CSV 导出、通知投递和网关 ACK 不再共享事件循环上的无界同步阻塞。
- 首次初始化、通知凭据响应和在线恢复的会话/Token 清理边界得到加固；通知渠道列表不再向浏览器返回 Token、密码或 Webhook 等敏感配置。
...
- Server 数据库 schema 从 v2 顺序迁移到 v19，启动前自动写入 `hub.db.v<旧版本>.bak` 快照；迁移包含原始 PDU/数据短信诊断、SIM 生命周期、设备网络与供电状态、事件流代际、持久化通知 outbox、任务同步和命令作业结果。当前 Agent/Server WebSocket 协议仍为 v1；部署时必须从同一提交升级并重启 Server 与 Agent，只同步 Agent 源码而不重新执行 `pip install` 仍会运行旧副本。
