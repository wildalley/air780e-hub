...
首次 `/auth/setup` 现在使用 `INSERT ... ON CONFLICT DO NOTHING` 认领密码设置项。scrypt
哈希在条件写入前完成，只有成功插入的请求会建立首个管理员会话；并发请求不会覆盖已存哈希，
而是返回 409。密码修改仍走显式的已认证路径并撤销已有会话。
...
渠道列表、创建和更新响应现在只返回非敏感配置，并用 `secret_fields` 标记已保存的 Token、
密码、Webhook/URL 等字段。更新请求省略敏感字段时保留数据库中的原值；`clear_secrets` 提供
显式清除语义，前端编辑器提供对应开关。通知发送器仍从数据库内部原始配置读取凭据，因此
脱敏不会改变实际投递。该实现不承诺静态加密，也未完成代理 DNS/重定向策略和生产密钥轮换演练。
...
新增公开的 `/readyz`。它在线程池中读取 settings 表，检查生命周期后台任务、通知投递器与维护状态；
失败时返回 503、`Retry-After: 2` 和逐项 `checks`，成功时返回 200。Agent 连接数只作为诊断字段，
不会阻止没有模块在线的 Server 进入就绪。Docker Compose 的容器检查已切换到 `/readyz`，
`/healthz` 继续用于轻量存活探测。维护期间跳过数据库读取，checks 全部为 true 才视为就绪。
部署自检器同时检查两个入口；数据库故障、后台任务缺失、通知停止和维护状态均有 503 用例。

#### 2026-09-06 已实现的第一批运行指标

已在现有管理员接口 `/api/operations/diagnostics` 的 `runtime.metrics` 增加进程内指标，响应
明确 `Cache-Control: no-store`。API 按框架匹配的路由模板、标准 HTTP 方法、状态码及完成状态
聚合；未知路径和维护前置拒绝归入固定类别。流式导出到最后一段响应发送完成才计时，异常和
取消另有状态，不缓存正文，也不把号码、设备名、查询参数或异常文本写进指标。

后台每秒采样 loop lag；数据库采集准入和执行器排队、worker 执行、最外层共享锁等待/持有、
只读连接会话和事件事务耗时，以及 SQLite BUSY/LOCKED 次数。提交与重复计数在 COMMIT 后
更新，即使等待者被取消也保留真实提交结果；网关从进入入站处理到成功发送 ACK 计时，发送
失败独立计数。既有事务回滚、提交后 ACK、取消等待和流式读取语义保留。

每项分位数只使用最近 256 次样本；次数、总耗时和最大耗时为进程累计。HTTP 最多保留 256
组标签，超限请求进入无标签的 `http_overflow` 聚合。指标跨在线恢复保留，采样任务随维护暂停、
恢复重启及应用停机协调。具体字段、测量边界及验证记录见[性能文档](performance.md#运行指标口径2026-09-06)。

#### 2026-09-07 已实现的第二批运行指标

通知投递现在记录 `notify_send` provider 调用耗时，并用固定计数器区分尝试、成功、失败、取消
和已安排重试。该边界不包含渠道查询、队列等待或结算写入；停用渠道和来源已删除等未发起
provider 调用的情况不会伪装成一次投递。网关关闭记录增加受控的 `gateway_closes` 分布，按
固定原因类别和规范化 WebSocket code 统计，最多保留 32 组，异常类别进入 `other`，不保存
Agent ID、close reason 或对端文本。认证失败、维护、自检、协议错误、重复 Agent、内部错误和
对端断开均有对应分类。

本批不改变 schema 或协议。仍未实现逐 SQL 计时、Agent 本地积压上报、作业阶段指标、
request_id 日志关联或外部指标存储；这些按本节规划继续推进。
...
`overview.counters.messages_today` 使用配置时区当天的半开 UTC 区间；消息趋势同样按 IANA 时区
分桶，并通过 `X-Hub-Calendar-*` 响应头返回实际时间窗和 `local-day` 桶标识。前端按 overview 的
时区和窗口结束时间补零及显示日期，不再使用浏览器本地日期推导；趋势卡明确展示日历时区。
上海跨日/跨月/跨年、纽约 23/25 小时日界、无卡与有卡同时存在的桶以及响应头均有回归测试。
无效配置时区沿用 UTC 回退，元数据返回实际采用的 UTC。IANA 转换在只读游标遍历时执行，
保留时间索引但增加 Python CPU 成本；100k 基准仍在原预算内，详见[性能记录](performance.md)。
...
2026-09-06：CLI 已显式设置 `workers=1`，部署自检拒绝多 worker 环境配置，部署文档明确单实例
约束。该检查不能自动发现外部编排复制的实例，仍需按部署清单核对副本数。
