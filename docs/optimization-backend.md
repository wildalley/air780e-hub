...
- `async def restore` 中同步写文件、验证和执行 backup；大文件可能阻塞事件循环。
...
Database 使用一个同步连接和 RLock。普通 `def` 路由通常在线程池运行，但网关入站、异步路由、审计中间件和 housekeeping 直接调用同步 DB。慢查询在线程里持有共享锁时，网关拿锁也可能阻塞事件循环；不能笼统地说“所有 API 都阻塞 loop”。FastAPI 不会自动把在 async 函数内调用的普通工具函数移入线程池。[FastAPI 并发说明](https://fastapi.tiangolo.com/async/)
...
- 默认全局最多 200 条、单队列最多 16 条，包含执行中命令；最多 32 个执行队列。队列满时返回明确的未执行回执，无回执的任务同步通过 `1013` 重连重新获取。
...
B06 的持久化作业、跨进程命令幂等和迟到结果查询仍待实现。B08 的 9.1 数据库调度与
9.3 任务 revision 回执也仍为待办。

### 9.3 任务同步闭环

目前 Server 在 hello 和修改任务时发送全量 `sync_tasks`，没有应用版本回执；发送失败主要等待下次重连。增加配置 revision/hash，Agent 完成本地事务后回报 applied revision；Server 展示 pending/applied/failed，并定期重发未应用版本。

不能在负责读取回执的 receive loop 中同步等待自己的回执；采用独立状态推进。任务执行产生 run_id，避免重复手动触发或重连同步重新执行同一计划实例。
...
| 保留清理 | 一次 DELETE 删除符合条件的全部行 | 分批删除并观测锁、WAL、剩余空间，支持中断后继续 |
