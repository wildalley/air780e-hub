...
> 状态:M0–M5 完成,252 个测试通过。硬件在途,M6 待硬件到货。
...
| D8 | **udev 按 USB 端口路径绑定设备名** | 两个同型号模块序列号极可能都是 `000000000001`,`by-id` 会撞车 |
...
### M6 — 硬件接入 🔌 *(需硬件)*
- 走 `docs/at-reference.md` §4 验证清单,回填待验证项
- 确认 AT 固件,必要时用 LuaTools 刷
- udev 规则固定 `air780e-a` / `air780e-b`
- ModemManager 处理
- 顺带测一次 `mmcli` 能否识别(记录结论,便于日后判断)
- **验收**:两个模块同时在线,真实收发通
