...
没有被任何 `[[devices]]` 认领的模块，Agent 会自己接管，设备名按 IMEI 后六位生成
（例如 `auto-372050`）。所以换模块不必先读 IMEI 再改配置：插上就能收短信，配置里
一个块都不写也可以。

| 参数 | 默认值 | 作用 |
| --- | ---: | --- |
| `autodetect` | `true` | 是否接管未被配置认领的模块 |
| `autodetect_interval` | `60.0` | 多久扫一次新插入的模块（秒） |

只有配置里所有设备都认不出来的模块才有资格被接管 —— 某个 `[[devices]]` 正在等的
模块（此刻拔着、或还在枚举）不会被顶掉，否则保号短信会从错误的卡发出去。名字存在
Agent 本地 SQLite 里，重启后不变：Server 按设备名归档历史，改名等于把一块模块的
历史劈成两个设备。

自动接管的设备和手写的一样可以下发命令、跑保号任务。想固定名字或标签，照常写一个
`[[devices]]` 块把它认领回来即可。
...
供电告警阈值写在**每个 `[[devices]]` 块内**而不是 `[agent]`：多低算低是这块模块供电
方式的属性，一台接实验室电源、一台经 USB hub 拉长线，健康区间并不相同。

| 参数 | 默认值 | 作用 |
| --- | ---: | --- |
| `low_voltage_mv` | `3500` | 低于该供电电压（毫伏）在运维中心告警；低于 3300 升为 critical。`0` 表示用内置默认值 |

阈值随状态帧上报给 Server，Server 只做判定、不另存一份默认值，避免两端对同一个
电压有不同意见。固件拒答 `AT+CBC` 时电压为空，已有告警保持不动 —— 沉默不算恢复。
...
断网、Agent/Server 重启、重复事件、USB 重新枚举和自愈限频另有可调循环次数的
[自动化故障注入基线](reliability.md)。自动化结果不能替代第 3、4、7、8 项的真实硬件
验收，但应在发布前先通过，以区分软件回归与现场环境问题。
...
模块在线、其他卡正常，只有一张卡的出站短信全部失败。先分清是本地问题还是网络侧
不放通，再决定动手方向 —— 这类失败绝大多数不在本项目侧。

设备详情的「网络诊断」和发送失败提示已带上注册域与固件版本。若要进一步确认，模块
的三个 ACM 口中 Agent 只占第一个，可用第三个口旁路查询而不打断收发（`02/04/06`
布局见 [AT 笔记 §1.1](at-reference.md#11-参考枚举结果)）。逐项确认短信中心
（`AT+CSCA?`）、`AT+CPIN?`、`AT+CREG?` / `AT+CGATT?`、`AT+CSQ`、`AT+CFUN?`、
`AT+CSMS?` 与存储余量（`AT+CPMS?`）。

以上全部正常、同一模块入站也正常、且对端始终收不到内容时，说明消息没有进入网络，
应到运营商侧确认该资费是否放通（漫游）MO 短信，而不是先升级固件。实测排除过程见
[AT 笔记 §2.1](at-reference.md#2026-08-18-出站失败的逐项排除)。注意 Air780E `V1011`
不支持 `AT+CEER`，拿不到网络侧 cause code，排除法就是能做到的上限。
