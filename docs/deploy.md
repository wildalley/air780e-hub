...
系统会为以下六种情况创建事件：模块离线、网络未注册、短信发送失败、保号任务
失败、通知投递失败和 Agent / Server 版本不一致。事件按模块、任务、渠道或 Agent
聚合，重复发生只累加次数，不会产生新行。管理员可以确认或解决事件；对应状态恢复时
（模块重新在线、注册成功、发送成功、任务成功、通知成功、两端版本恢复一致），系统
会自动解决仍未关闭的事件。模块离线时，它的
