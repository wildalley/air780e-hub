...
下一工作包随后推进 B06 持久化命令作业；B07 恢复维护模式的自动化实现见下节，真实部署组合负载验收仍待完成。

### 2026-09-06：持久化命令作业

B06 第一版已完成：Server schema 升至 19，作业状态、幂等键、请求摘要、截止时间、设备及
连接/事件流身份均持久化；`POST /api/operations` 返回 202，状态可通过作业资源和运维中心
查询。新 Agent 增加 `command_runs` journal，支持重复 cmd_id 的结果重放，并将重启中已
开始的短信、呼叫等操作标为 `unknown`。迟到回执需匹配 `result_token`，不会覆盖已确认结果。
手动任务的 `run_id` 与 operation_id 一致并写入任务日志；旧 action 接口保留兼容路径。

验证：Server 全量 289 项通过，新增 2 项作业幂等/结果测试；Agent 62 项相关测试通过；
前端 164 项测试与 TypeScript 类型检查通过；Python Ruff 与 `git diff --check` 通过。
尚未做真实硬件断连、代理超时和跨进程 Server 重启演练；B07 恢复维护模式的自动化实现见下节。

### 2026-09-06：B07 恢复维护模式与可回退切换

B07 已完成隔离自动化实现。恢复先以大小、上传超时和剩余空间预算接收候选备份，在私有
SQLite 文件中完成迁移、完整性、外键、schema 和任务/设备引用校验；候选失败时在线库不变。
进入维护后，新的 API 请求返回 503（健康检查与恢复状态查询除外），后台任务、告警和通知
投递暂停，已建立 Agent 连接收到 `1012`，旧 Gateway 被退休，延迟注销不能覆盖恢复后新连接的
在线状态。

drain 仅等待已准入的 HTTP 请求，不依赖客户端主动结束 WebSocket task。切换前将 Agent 标为
离线、运行中作业标为 `unknown`、未下发作业取消，并写入私有恢复前快照；成功后清除 session 和
旧 Token 宽限，重置任务同步、通知租约和运行时在线标记，重建认证、通知、告警与 Gateway。
切换或校验失败会从快照回退；切换与回退均失败时保持维护模式并返回可供人工恢复的快照路径。
每次成功恢复保留最近 3 份私有快照。

验证：`server/` 中 `.venv/bin/pytest -q -o faulthandler_timeout=20 --tb=short` 为 305 passed
（1 条既有 Starlette TestClient 弃用警告），`.venv/bin/ruff check src tests benchmarks` 和
`git diff --check` 通过。覆盖超限、磁盘不足、坏引用、切换/回退失败、取消、维护请求阻断、
会话与 Token 清理、快照保留、Agent `1012` 关闭、重连和延迟旧连接清理。

未进行真实部署、真实硬件断连、外部通知发送或生产恢复演练；RPO 仍由备份频率决定，已 ACK 且
被 Agent 清除的历史不会因恢复自动重建。

### 2026-09-06：B10 首次初始化与通知凭据边界

B10 本轮补齐了两处可直接验证的边界：首次管理员密码使用条件插入原子认领，哈希仍在写入前
计算，竞争请求不会覆盖已配置密码并返回 409；通知渠道列表和写入响应不再返回 Token、密码、
Webhook/URL 等凭据，返回 `secret_fields` 元数据。更新渠道时省略敏感字段表示保留原值，
`clear_secrets` 才会显式清除；前端编辑器支持替换或清除已保存凭据。
