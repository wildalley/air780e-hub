...
Agent 收帧循环立即处理 `ack` 和 `resend_from`,硬件命令交给独立的有界队列。同一设备
按接收顺序执行,不同设备可以并行;`sync_tasks` 和 `run_task` 共用控制队列,保证配置先于
紧随其后的手动运行生效。底层 modem 的 AT 串行约束继续有效。
...
- `1013` Agent 命令队列已满且无法接收无回执的任务同步;退避重连后重新同步配置
